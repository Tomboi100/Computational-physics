...
def solveDiffusion1D_withCellCenteredGrid(Nx=20, Nt=50, alpha=0.5, dt=0.01, xmin=0.0, xmax=1.0,
                                          ...
                                          upperBoundary=lambda t: 0., method=hybrid, **kwargs):
    # Calculate dx based on cell-centered grid approach
    dx = (xmax - xmin) / Nx

    # Define the cell-centered grid points
    ...
    # Calculate the mu parameter for the cell-centered grid
    mu = alpha * dt / (dx ** 2)

    # Use the provided hybrid method to compute the solution
...
# Example usage
x, t, v = solveDiffusion1D_withCellCenteredGrid(initialConditions=lambda x: np.sin(np.pi * x),
                                                lowerBoundary=lambda t: f(0.),
                                                upperBoundary=lambda t: f(1.),
...
solve = solveDiffusion1D_withCellCenteredGrid
results = []
for m in methods:
    if type(m) is tuple:
